...
function jailbreakCountdown() {   
    ...
    let countdown = 5;
    label.textContent = `Auto Jailbreak em: ${countdown}`;
    timerId = setInterval(() => {
        countdown--;
        label.textContent = `Auto Jailbreak em: ${countdown}`;
...
function startAutoJb() {
    ...
    var onCacheError = function () {
        ...
        if (retries < MAX_CACHE_RETRIES) {
            label.textContent = 'Erro de cache - tentando novamente (' + (retries + 1) + '/' + MAX_CACHE_RETRIES + ')...';
            setTimeout(function () {
        ...
        label.textContent = 'Erro de cache - pressione Jailbreak manualmente';
    };
    ...
    var onLateDownload = function () {
        ...
        stopInterval();
        label.textContent = 'Atualização de cache encontrada... início automático pausado';
